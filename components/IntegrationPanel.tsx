...
const IntegrationPanel: React.FC<Props> = ({ isOpen, onClose, onImport }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex justify-end bg-orange-900/20 backdrop-blur-sm transition-all">
      <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-500">
        <div className="p-8 border-b border-[#fed7aa] flex items-center justify-between gradient-bg text-white">
          <div>
        ...
        <div className="flex-1 p-8 space-y-4 overflow-y-auto no-scrollbar">
          ...
          {sources.map((source) => (
            <div key={source.id} className="p-5 rounded-[1.5rem] border border-[#fed7aa] bg-gray-50 flex items-center justify-between hover:border-orange-400 transition-all group">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-black text-[#9a3412] text-sm uppercase tracking-tight">{source.name}</h4>
              ...
              <button 
                ...
                className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${
                  source.connected 
                    ? 'bg-orange-50 text-[#9a3412] hover:bg-orange-100 border border-orange-200'
        ...
        <div className="p-8 bg-orange-50 border-t border-[#fed7aa]">
          <div className="flex items-center gap-3 text-[#f97316] text-xs font-black uppercase tracking-widest mb-4">
            <CheckCircle size={18} />
